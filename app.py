...
def _is_policy_schedule_active(policy, now_ts=None):
    ...
    start_h, start_m = _parse_hhmm(sched.get("start"))
    end_h, end_m = _parse_hhmm(sched.get("end"))
...
def _select_active_policy(data, student_email):
    ...
    user_map = assigns.get("users", {}) or {}
    group_map = assigns.get("groups", {}) or {}
    ...
    if student_email and student_email in user_map:
        pid = user_map.get(student_email)
        if pid:
    ...
    for cid, cls in (data.get("classes") or {}).items():
        try:
            ...
            students = []
        if student_email and student_email in students:
            pid = group_map.get(cid)
            if pid:
                applicable_ids.add(pid)
...
def api_policy():
    ...
    resp = {
        ...
        "active_policy": ({"id": active_policy.get("id"), "name": active_policy.get("name"), "priority": active_policy.get("priority", 0)} if active_policy else None),
...
def api_policy_assignments():
    ...
    for scope in ("users", "groups"):
        if scope in body and isinstance(body[scope], dict):
            for key, pid in body[scope].items():
                key = (key or "").strip()
                if not key:
                    continue
                if pid is None:
